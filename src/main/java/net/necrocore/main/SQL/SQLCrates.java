...
public class SQLCrates implements Listener{
	
	private static HyperialCore plugin;
	public SQLCrates(HyperialCore listener) {
		SQLCrates.plugin = listener;		
	}
    ...
    public static void loadPlayer(final Player p) {
        Bukkit.getScheduler().runTaskAsynchronously(plugin, new Runnable() {
            public void run() {
                try {
                    PreparedStatement statement = HyperialCore.mysql.prepareStatement(INSERT);
                    statement.setString(1, p.getUniqueId().toString());
                    statement.setString(2, p.getName());
                    statement.setInt(3, meteors.get(p.getUniqueId()));
                    statement.setString(4, p.getName());
                    HyperialCore.mysql.update(statement);

                    statement = HyperialCore.mysql.prepareStatement(SELECT);
                    statement.setString(1, p.getUniqueId().toString());
                    ResultSet result = HyperialCore.mysql.query(statement);
                    if (result.next())
    ...
    public static void savePlayer(final Player p) {
                try {
                	if(meteors.containsKey(p.getUniqueId())){
                    PreparedStatement statement = HyperialCore.mysql.prepareStatement(SAVE);
